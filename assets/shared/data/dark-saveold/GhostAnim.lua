...
function GhostAnim(Character,SingAnim)
    ...
    if not lowQuality then
        setBlendMode('Ghost'..Ghost,'EASYLIGHT')
    end
    playAnim('Ghost'..Ghost,SingAnim,true)
    doTweenAlpha('Ghost'..Ghost,'Ghost'..Ghost,0,0.75,'linear')
    if curStage=='finale' then
    setProperty('camGame.zoom',getProperty('camGame.zoom')+0.020)
    setProperty('camHUD.zoom',getProperty('camHUD.zoom')+0.020)
    end
end
